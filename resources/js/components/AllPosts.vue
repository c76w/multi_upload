<template>
	<div class="row">
		<div class="col-md-6" v-for="(post, i) in posts" :key=i>
			<div class="card mt-4">
				<img v-if="post.post_images.length" :src="post.post_images[0].post_image_path" class="card-img-top">
				<div class="card-body">
					<p class="card-text">
						<strong>{{ post.title }}</strong> <br> {{ truncateText(post.body) }}
					</p>
				</div>
			...
			<span>
				...
				<div class="row">
					<div class="col-md-6" v-for="(img, i) in currentPost.post_images" :key=i>
						<img :src="img.post_image_path" alt="" class="img-thumbnail">
					</div>
				</div>
				<hr>
